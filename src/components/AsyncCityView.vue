<template>
  <div class="flex flex-1 flex-col items-center">
    ...
    <div class="flex flex-col items-center py-12 text-white">
      ...
      <p class="mb-12 text-sm" v-if="weatherData">
        {{ formattedTime(weatherData.currentTime) }}
      </p>
      <p class="mb-8 text-8xl" v-if="weatherData && weatherData.hourly && weatherData.hourly.length > 0">
        {{ Math.round(weatherData.hourly[0].temperature) }}&deg;C
      </p>
      <p v-if="weatherData && weatherData.hourly && weatherData.hourly.length > 0">
        Feels like
        {{
      ...
      <img
        ...
        :src="
          getWeatherIcon(
            weatherData.current && weatherData.current.weather && weatherData.current.weather[0].icon
          )
        "
        alt=""
      />
    ...
    <div class="w-full max-w-screen-md py-12">
      <div class="mx-8 text-white">
        ...
        <div
          class="flex gap-10 overflow-x-scroll scrollbar-thin scrollbar-track-weather-secondary scrollbar-thumb-weather-tertiary"
        >
          <div
          ...
          >
            <p v-if="hourData.currentTime" class="text-mb whitespace-nowrap">
            ...
            <img
              ...
              alt=""
            />
            <p class="text-xl">{{ Math.round(hourData.temperature) }}&deg;</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="w-full max-w-screen-md py-12">
      <div class="mx-8 text-white">
        ...
        <div v-for="day in weatherData.daily" :key="day.dt" class="flex items-center">
          ...
          <img class="h-[50px] w-[50px] object-cover" :src="getWeatherIcon(day.weather[0].icon)" alt="" />
